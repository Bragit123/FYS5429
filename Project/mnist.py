...
for i in range(len(etas)):
    for j in range(len(lams)):
        ...
        network = create_convolutional_neural_network_our_code(CostLogReg, input_shape, n_hidden_neurons, act_func, scheduler, n_filters)
...
heatmap(train_accs, xticks=lams, yticks=etas, title=f"Training Accuracy, LRELU", xlabel="$\lambda$", ylabel="$\eta$", filename=f"../Figures/LRELU_TrainAccs6000IMGs.pdf")
heatmap(val_accs, xticks=lams, yticks=etas, title=f"Validation Accuracy, LRELU", xlabel="$\lambda$", ylabel="$\eta$", filename=f"../Figures/LRELU_ValAccs6000IMGs.pdf")